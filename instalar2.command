...
async function loadPlayers(){try{players=await kget('players')||[];kickoffs={...KICKOFFS_DEFAULT,...(await kget('kickoffs')||{})};if(!players.length){showToast('⚠️ No hay jugadores aún',true);return;}showScreen('select');document.getElementById('playerList').innerHTML=players.map(p=>`<button class="player-opt" onclick="selectPlayer('${p}')">${p}</button>`).join('');}catch(e){showToast('❌ '+e.message,true);}}
...
<div id="tab-jugadores" class="tab-panel active">
  <div class="card"><h2>👥 Jugadores</h2><p class="hint">Hasta 12. Escribí los nombres y guardá.</p><div class="pgrid" id="pGrid"></div>
...
const FECHAS=buildFechas();
let STATE={players:[],results:{},pts:{},fechaWinners:{},allPreds:{},kickoffs:{}};
...
async function loadAll(){try{STATE.players=await kget('players')||[];STATE.results=await kget('results')||{};STATE.pts=await kget('pts')||{};STATE.fechaWinners=await kget('fechaWinners')||{};STATE.kickoffs={...KICKOFFS_DEFAULT,...(await kget('kickoffs')||{})};STATE.allPreds={};for(const p of STATE.players){STATE.allPreds[p]=await kget(`preds:${p}`)||{};}renderSetup();showToast('✅ Datos cargados');}catch(e){showToast('❌ '+e.message,true);}}
function renderSetup(){const g=document.getElementById('pGrid');g.innerHTML='';for(let i=0;i<12;i++){const inp=document.createElement('input');inp.type='text';inp.className='inp';inp.placeholder=`Jugador ${i+1}`;inp.value=STATE.players[i]||'';inp.dataset.i=i;g.appendChild(inp);}
...
names.forEach((name,ni)=>{const preds=STATE.allPreds[name]||{};const pred=preds[rk];if(!pred||pred.h===''||pred.a==='')return;const ph=parseInt(pred.h),pa=parseInt(pred.a),pw=ph>pa?'H':pa>ph?'A':'D';if(ph===rh&&pa===ra)STATE.pts[`${rk}_${ni}`]=3;else if(pw===rw)STATE.pts[`${rk}_${ni}`]=1;else STATE.pts[`${rk}_${ni}`]=0;});});
...
HTMLEOF2
echo "Copiando a public/ (donde Vercel sirve)..."
mkdir -p public
cp index.html public/index.html
cp admin.html public/admin.html
echo ""
echo "=== Verificacion ==="
for target in index.html admin.html public/index.html public/admin.html; do
  if grep -q "GRP_PARTIDOS" "$target" && grep -q "epic-possum-110301" "$target"; then
    echo "OK: $target (fixture real + base correcta)"
  else
    echo "ERROR en $target"
  fi
done
echo ""
echo "=== Desplegando ==="
git add -A
git commit -m "Fixture real Mundial 2026 con kickoffs y bloqueo"
git push
npx vercel --prod --yes
echo ""
echo "=== LISTO ==="
