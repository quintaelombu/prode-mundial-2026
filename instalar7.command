...
<div id="tab-tabla" class="tab-panel">
  <div class="flex" style="margin-bottom:14px;justify-content:space-between;flex-wrap:wrap;gap:10px">
  <div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:var(--gold);letter-spacing:3px">TABLA — FECHA ACTUAL</div>
  <button class="btn sm" onclick="calcularTodo()">🔄 Actualizar</button>
</div>
<div class="card" style="margin-bottom:14px">
  <h2>📅 Fecha en juego</h2>
  <p class="hint">Seleccioná la fecha que está en juego ahora. La tabla mostrará solo los puntos de esa fecha.</p>
  <select id="fechaActivaSelect" class="inp" style="margin-bottom:12px" onchange="cambiarFechaActiva(this.value)">
    <option value="">— Seleccioná una fecha —</option>
  </select>
  <div id="fechaActivaLabel" style="font-size:0.85rem;color:var(--muted)"></div>
</div>
...
const FECHAS=buildFechas();
let STATE={players:[],results:{},pts:{},fechaWinners:{},allPreds:{},kickoffs:{},pagos:{},fechaActiva:null};
...
async function loadAll(){try{STATE.players=await kget('players')||[];STATE.results=await kget('results')||{};STATE.pts=await kget('pts')||{};STATE.fechaWinners=await kget('fechaWinners')||{};STATE.kickoffs={...KICKOFFS_DEFAULT,...(await kget('kickoffs')||{})};
    STATE.pagos=await kget('pagos')||{};
    STATE.fechaActiva=await kget('fechaActiva')||null;STATE.allPreds={};for(const p of STATE.players){STATE.allPreds[p]=await kget(`preds:${p}`)||{};}renderSetup();showToast('✅ Datos cargados');}catch(e){showToast('❌ '+e.message,true);}}
...
[FECHAS.find(f=>f.id===STATE.fechaActiva)].filter(Boolean).forEach(fecha=>{const ap=APUESTAS[fecha.tipo];html+=`<div class="fase-header"><span class="fase-title">${fecha.label}</span><span class="fase-apuesta">💰 Pozo: $${(ap*names.length).toLocaleString()}</span></div>`;let cg='';
fecha.partidos.forEach((m,mi)=>{if(m.grupo!==cg){cg=m.grupo;if(fecha.id.startsWith('GRP'))html+=`<div class="grupo-label">— ${m.grupo} —</div>`;}
...
async function calcularTodo(){
  if(!STATE.fechaActiva){showToast('❌ Seleccioná la fecha activa primero',true);return;}
  const names=STATE.players;
  // Limpiar solo los pts de la fecha activa
  Object.keys(STATE.pts).forEach(k=>{if(k.startsWith(STATE.fechaActiva))delete STATE.pts[k];});
  delete STATE.fechaWinners[STATE.fechaActiva];
...
function initFechaSelect(){
  const sel=document.getElementById('fechaActivaSelect');
  sel.innerHTML='<option value="">— Seleccioná una fecha —</option>';
  FECHAS.forEach(f=>{const o=document.createElement('option');o.value=f.id;o.textContent=f.label+' ($'+APUESTAS[f.tipo].toLocaleString()+')';if(STATE.fechaActiva===f.id)o.selected=true;sel.appendChild(o);});
  updateFechaLabel();
}
async function cambiarFechaActiva(fid){
  STATE.fechaActiva=fid||null;
  await kset('fechaActiva',STATE.fechaActiva);
  updateFechaLabel();
  renderTabla();
}
function updateFechaLabel(){
  const lbl=document.getElementById('fechaActivaLabel');if(!lbl)return;
  if(!STATE.fechaActiva){lbl.textContent='Sin fecha seleccionada — la tabla no mostrará datos.';return;}
  const f=FECHAS.find(x=>x.id===STATE.fechaActiva);
  lbl.innerHTML=f?`Mostrando tabla de <strong style="color:var(--gold)">${f.label}</strong>. La tabla arranca de cero para esta fecha.`:'';
}
function renderTabla(){const names=STATE.players;
  if(!STATE.fechaActiva){document.getElementById('tablaBody').innerHTML='<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:28px">Seleccioná la fecha activa arriba para ver la tabla.</td></tr>';return;}
  const fechaActual=FECHAS.find(f=>f.id===STATE.fechaActiva);
  if(!fechaActual){document.getElementById('tablaBody').innerHTML='<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:28px">Fecha no encontrada.</td></tr>';return;}
  const scores=names.map((name,ni)=>{let pts=0,ex=0,win=0,fg=0,din=0;
  fechaActual.partidos.forEach((_,mi)=>{const p=STATE.pts[`${fechaActual.id}_${mi}_${ni}`];if(p===3){pts+=3;ex++;}else if(p===1){pts+=1;win++;}});
  const ws=STATE.fechaWinners[fechaActual.id];if(ws)ws.forEach(w=>{if(w.ni===ni){fg++;din+=w.amount;}});
  return{name,pts,ex,win,fg,din};});
...
echo ""
echo "=== Verificacion ==="
if grep -q "fechaActiva" public/admin.html; then echo "OK: tabla por fecha activa"; else echo "ERROR"; fi
if grep -q "epic-possum-110301" public/index.html; then echo "OK: base correcta"; else echo "ERROR base"; fi
echo ""
echo "=== Desplegando ==="
git add -A
git commit -m "Tabla por fecha activa independiente"
git push
npx vercel --prod --yes
echo ""
echo "=== LISTO ==="
