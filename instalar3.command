...
let currentPlayer=null,myPreds={},players=[],kickoffs={},pagos={};
...
echo ""
echo "=== Verificacion ==="
for target in index.html public/index.html; do
  if grep -q "pago-fecha-monto" "$target" && grep -q "epic-possum-110301" "$target"; then
    echo "OK: $target (pago por fecha + base correcta)"
  else echo "ERROR en $target"; fi
done
for target in admin.html public/admin.html; do
  if grep -q "renderPagos" "$target" && grep -q "STATE.pagos" "$target"; then
    echo "OK: $target (pestana pagos)"
  else echo "ERROR en $target"; fi
done
echo ""
echo "=== Desplegando ==="
git add -A
git commit -m "Pago por fecha con aprobacion del admin"
git push
npx vercel --prod --yes
echo ""
echo "=== LISTO ==="
